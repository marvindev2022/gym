...
export function WorkoutNewPage() {
  ...
  function adicionarExercicios(g: string) {
    if (!g || !EXERCICIOS_POR_GRUPO[g]) return
    EXERCICIOS_POR_GRUPO[g].forEach(ex => append({
      name: ex.name,
      sets: String(ex.sets),
      reps: ex.reps,
  ...
  function substituirTudo(g: string) {
    if (!g || !EXERCICIOS_POR_GRUPO[g]) return
    replace(EXERCICIOS_POR_GRUPO[g].map(ex => ({
      name: ex.name,
      sets: String(ex.sets),
      reps: ex.reps,
      rest_seconds: String(ex.rest_seconds),
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
        ...
        <div className="tz-card p-4 flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {DIVISOES.map(d => (
                <button key={d} type="button"
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors active:scale-95 ${divisao === d ? 'bg-tz-gold text-tz-bg' : 'bg-tz-surface-2 border border-tz-border text-tz-muted hover:text-tz-white'}`}
                >{d}</button>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {GRUPOS_MUSCULARES.map(g => (
                <button key={g} type="button"
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors active:scale-95 ${grupo === g ? 'bg-tz-electric text-tz-bg font-bold' : 'bg-tz-surface-2 border border-tz-border text-tz-muted hover:text-tz-white'}`}
                >{g}</button>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {OBJETIVOS.map(o => (
                <button key={o} type="button"
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors active:scale-95 ${objetivo === o ? 'bg-tz-surface border border-tz-gold text-tz-gold' : 'bg-tz-surface-2 border border-tz-border text-tz-muted hover:text-tz-white'}`}
                >{o}</button>
              ))}
            </div>
          </div>
          ...
          {grupo && EXERCICIOS_POR_GRUPO[grupo] && (
            <div className="flex gap-2">
              <button type="button" onClick={() => adicionarExercicios(grupo)}
                className="flex-1 flex items-center justify-center gap-2 rounded-tz bg-tz-electric/10 border border-tz-electric/30 py-3 text-sm font-bold text-tz-electric hover:bg-tz-electric/20 active:scale-95 transition-all"
              >
                ⚡ Adicionar {EXERCICIOS_POR_GRUPO[grupo].length} exercícios de {grupo}
              </button>
              {fields.length > 0 && (
                <button type="button" onClick={() => substituirTudo(grupo)}
                  className="px-3 rounded-tz border border-tz-border text-xs text-tz-muted hover:text-tz-white hover:border-tz-muted active:scale-95 transition-all"
                  title="Substituir todos os exercícios atuais"
                >
                  Substituir
                </button>
              )}
        ...
        <div className="flex flex-col gap-3">
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="tz-card p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-tz-gold">Exercício #{index + 1}</span>
                <button type="button" onClick={() => remove(index)} className="text-xs text-tz-muted hover:text-tz-error transition-colors active:scale-95">Remover</button>
              </div>
              <div className="relative">
                <input
                  ...
                  {...register(`exercises.${index}.name`, {
                    onChange: (e) => handleExerciseInput(index, e.target.value),
                  })}
                />
                {(autocomplete[index]?.length ?? 0) > 0 && (
                  <div className="absolute top-full left-0 right-0 z-20 mt-1 rounded-tz-sm border border-tz-border bg-tz-surface-2 shadow-tz-card overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-2">
                {([
                  ['Séries', 'sets', '3', 'number'],
                  ['Reps', 'reps', '10-12', 'text'],
                  ['Descanso (s)', 'rest_seconds', '60', 'number'],
                ] as const).map(([label, f, ph, type]) => (
                  <div key={f} className="flex flex-col gap-1">
                    ...
                    <input type={type} placeholder={ph} min="0"
                      ...
                      {...register(`exercises.${index}.${f}`)}
                    />
                  </div>
                ))}
              </div>
              <input
                placeholder="Observação (ex: mantenha o cotovelo fixo)"
          ...
          <button
            type="button"
            onClick={() => append({ name: '', sets: '3', reps: '10-12', rest_seconds: '60', notes: '' })}
            className="flex items-center justify-center gap-2 rounded-tz-sm border border-dashed border-tz-border py-3 text-sm text-tz-muted hover:border-tz-gold hover:text-tz-gold active:scale-95 transition-all"
          >
        ...
        <div className="flex gap-3 pb-6">
          ...
          <Button type="submit" fullWidth isLoading={isSubmitting} disabled={fields.length === 0}>
            Criar treino {fields.length > 0 && `(${fields.length} exercícios)`}
          </Button>
        </div>
      </form>
    </div>
